...
BEGIN
	SET @form_id = (SELECT id FROM `%TABLE_PREFIX%form` WHERE title='Stock' LIMIT 1);
	SET @status_list_id = (SELECT id FROM `%TABLE_PREFIX%list` WHERE `name`='Stock_status' LIMIT 1);
	SET @Stock_list_id = (SELECT id FROM `%TABLE_PREFIX%list` WHERE `name`='Stock' LIMIT 1);
...
CREATE PROCEDURE `%TABLE_PREFIX%UpgradeStockFormFields`()
BEGIN
	SET @form_id = (SELECT id FROM `%TABLE_PREFIX%form` WHERE title='Stock' LIMIT 1);
	SET @status_list_id = (SELECT id FROM `%TABLE_PREFIX%list` WHERE `name`='Stock_status' LIMIT 1);
	SET @Stock_list_id = (SELECT id FROM `%TABLE_PREFIX%list` WHERE `name`='Stock' LIMIT 1);
...
call `%TABLE_PREFIX%UpgradeStockFormFields`()$
